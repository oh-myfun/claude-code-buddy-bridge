...
外部有一条路径同时运行：
  ...
  Claude Code CLI
      ...
      ▼
  TCP Socket  (localhost:9876) ◄─── TCP ─── 设备（TCP 客户端）
      │
      ▼
...
TCP_PORT_DEFAULT = 9876  # 设备连接端口（hook 也使用此端口）
...
class Bridge:
    def __init__(self) -> None:
        ...
        self._devices: Set[DeviceConnection] = set()
    ...
    def _is_hook_request(self, msg: dict) -> bool:
        """判断消息是否为 hook 请求"""
        # hook 请求包含 id 和 tool
        return "id" in msg and "tool" in msg
    ...
    async def handle_client(
        ...
        if self._is_hook_request(msg):
            ...
            await self._handle_hook(reader, writer, msg)
        elif self._is_device_message(msg):
            # 这是设备连接
            logger.info(f"[{addr}] 识别为设备连接")
            await self._handle_device(reader, writer, msg)
        else:
            # 未知类型
    ...
    async def _handle_device(
        ...
        try:
            # 处理第一条消息（已在调用前解析）
            if first_msg.get("cmd") == "permission":
                await self._handle_permission_decision(first_msg)
    ...
    async def _handle_hook(
        ...
        """处理 hook 连接"""
        rid = str(first_msg.get("id") or f"req_{int(time.time() * 1000)}")
        tool = str(first_msg.get("tool") or "?")
        hint = str(first_msg.get("hint") or "")
        context = first_msg.get("context") if isinstance(first_msg.get("context"), dict) else None
...
async def run() -> None:
    ...
    logger.info("  - hook 和设备共用此端口，通过协议自动识别")
    ...
    # 清理所有设备连接
    for device in list(bridge._devices):
        try:
            device.writer.close()
